...
impl LearningEngine {
    ...
    pub fn with_config(config: LearningConfig) -> Self {
        info!(
            "Initializing Learning Engine with {} max samples",
            config.max_learning_samples
        );
    ...
    pub async fn record_learning_data(&mut self, data: LearningData) -> Result<()> {
        debug!(
            "Recording learning data: {} success={}",
            data.action_type, data.success
        );
    ...
    async fn update_action_pattern(&mut self, data: &LearningData) -> Result<()> {
        ...
        pattern.average_execution_time =
            (1.0 - alpha) * pattern.average_execution_time + alpha * data.execution_time_ms as f64;
        ...
        debug!(
            "Updated pattern {}: success_rate={:.2}, confidence={:.2}",
    ...
    fn is_significant_parameter(&self, key: &str, _value: &serde_json::Value) -> bool {
        ...
        matches!(
            key,
            "selector" | "text" | "url" | "element_type" | "wait_time" | "timeout"
        )
    }
    ...
    fn generate_pattern_improvements(&self, pattern: &ActionPattern) -> Vec<String> {
        ...
        if pattern.success_rate < self.learning_config.success_threshold {
            suggestions
                .push("Success rate below threshold - consider alternative selectors".to_string());
        }

        if pattern.average_execution_time > 5000.0 {
            suggestions
                .push("Execution time high - consider optimizing wait conditions".to_string());
        }

        if pattern.total_attempts > 20 && pattern.success_rate < 0.5 {
            suggestions.push(
                "Frequent failures detected - review element identification strategy".to_string(),
            );
        }
    ...
    async fn update_performance_metrics(&mut self) {
        ...
        let average_confidence =
            self.learning_data.iter().map(|d| d.confidence).sum::<f64>() / total_samples as f64;
        ...
        debug!(
            "Updated performance metrics: success_rate={:.2}, patterns={}",
            success_rate,
    ...
    fn calculate_improvement_trend(&self) -> f64 {
        ...
        let recent_success = self
            ...
            .filter(|d| d.success)
            .count() as f64
            / (self.learning_data.len() - half_point) as f64;
    ...
    async fn generate_insights(&self, data: &LearningData) -> Vec<LearningInsight> {
        ...
        if !data.success {
            ...
            if recent_failures >= 3 {
                insights.push(LearningInsight {
                    ...
                    description: format!(
                        "Action '{}' failing repeatedly - may need strategy change",
                        data.action_type
                    ),
                    confidence: 0.8,
        ...
        if data.execution_time_ms > 10000 {
            // Slow execution
            insights.push(LearningInsight {
    ...
    pub async fn get_improvement_recommendations(&self, action_type: &str) -> Vec<String> {
        ...
        let high_performers: Vec<_> = patterns.iter().filter(|p| p.success_rate > 0.8).collect();

        if !high_performers.is_empty() {
            recommendations.push(format!(
                "Use successful patterns: {} high-performing variants found",
                high_performers.len()
            ));
        }
    ...
    pub async fn cleanup_old_data(&mut self) -> Result<usize> {
        ...
        self.action_patterns
            .retain(|_, pattern| pattern.last_updated > cutoff_date);
...
mod tests {
    ...
    async fn test_record_learning_data() {
        ...
        let data = LearningData {
            ...
            parameters: {
                ...
                params.insert(
                    "selector".to_string(),
                    serde_json::Value::String("button.submit".to_string()),
                );
                params
            },
        ...
        for i in 0..15 {
            let data = LearningData {
                ...
                parameters: {
                    ...
                    params.insert(
                        "selector".to_string(),
                        serde_json::Value::String("button.test".to_string()),
                    );
                    params
                },
    ...
    async fn test_improvement_recommendations() {
        ...
        assert!(recommendations
            .iter()
            .any(|r| r.contains("reliable selectors")));
    }
}
