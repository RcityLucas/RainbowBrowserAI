...
use crate::browser::{ElementInfo, BrowserOps};
...
impl OrganicPerceptionEngine {
    ...
    fn initialize_default_models(&mut self) {
        ...
        let ecommerce_model = PerceptionModel {
            ...
            element_patterns: {
                ...
                patterns.insert("add_to_cart".to_string(), ElementPattern {
        ...
        self.perception_models.insert("e-commerce".to_string(), ecommerce_model);
        ...
        let social_model = PerceptionModel {
            ...
            element_patterns: {
                let mut patterns = HashMap::new();
                patterns.insert("like_button".to_string(), ElementPattern {
                    element_type: "button".to_string(),
                    selectors: vec![
                    ...
                    context_clues: vec!["post context".to_string(), "social interaction".to_string()],
        ...
        self.perception_models.insert("social-media".to_string(), social_model);
        ...
        let form_model = PerceptionModel {
            ...
            element_patterns: {
                let mut patterns = HashMap::new();
                patterns.insert("submit_button".to_string(), ElementPattern {
                    element_type: "button".to_string(),
                    selectors: vec![
                    ...
                    context_clues: vec!["form context".to_string(), "input fields nearby".to_string()],
        ...
        self.perception_models.insert("forms".to_string(), form_model);
    }
    ...
    pub async fn analyze_page_deeply(
        ...
        info!("Starting deep organic perception analysis for: {}", page_context.url);
        
        // 1. Identify applicable models
        let applicable_models = self.get_applicable_models(&page_context.domain);
        debug!("Found {} applicable models for domain: {}", applicable_models.len(), page_context.domain);
        
        // 2. Get page characteristics
        let page_characteristics = self.analyze_page_characteristics(browser).await?;
        debug!("Page characteristics: SPA={}, Dynamic={}", page_characteristics.is_spa, page_characteristics.dynamic_loading);
        ...
        let context_elements = self.apply_contextual_perception(page_context, browser).await?;
        ...
        if self.learning_enabled {
            self.update_context_memory(page_context, &page_characteristics).await;
        }
    ...
    async fn analyze_page_characteristics(&self, browser: &crate::browser::Browser) -> Result<PageCharacteristics> {
            ...
            for selector in &pattern.selectors {
                if let Ok(nodes) = browser.find_elements(selector).await {
                    for element in nodes {
                        if let Some(insight) = self.create_element_insight(
                            &element,
                            selector,
                            element_type,
    ...
    async fn apply_contextual_perception(
        ...
        let contextual_selectors = if page_context.url.contains("login") || page_context.title.to_lowercase().contains("login") {
            vec![
        ...
        } else if page_context.url.contains("search") || page_context.title.to_lowercase().contains("search") {
            vec![
        ...
        for (selector, element_type) in contextual_selectors {
            if let Ok(nodes) = browser.find_elements(selector).await {
                for element in nodes.into_iter().take(10) { // Limit to avoid overwhelming results
                    if let Some(insight) = self.create_element_insight(
                        &element,
                        selector,
                        element_type,
    ...
    async fn apply_learned_perception(
        ...
        for (selector, success_rate) in &site_context.successful_selectors {
            if *success_rate > 0.6 { // Only use relatively successful selectors
                if let Ok(nodes) = browser.find_elements(selector).await {
    ...
    fn predict_element_behavior(&self, _element: &ElementInfo, element_type: &str) -> Option<String> {
        match element_type {
    ...
    fn calculate_page_complexity(&self, characteristics: &PageCharacteristics, element_count: usize) -> f64 {
        ...
        if characteristics.is_spa { complexity += 0.2; }
        if characteristics.dynamic_loading { complexity += 0.2; }
        if characteristics.has_shadow_dom { complexity += 0.1; }
        if characteristics.uses_react || characteristics.uses_angular { complexity += 0.1; }
    ...
    async fn update_context_memory(&mut self, page_context: &PageContext, characteristics: &PageCharacteristics) {
        let domain = page_context.domain.clone();
        
        let site_context = self.context_memory.site_contexts
            .entry(domain.clone())
        ...
        if self.context_memory.site_contexts.len() > self.context_memory.max_memory_entries {
            ...
            let oldest_domains: Vec<String> = self.context_memory.site_contexts
                .iter()
    ...
    pub async fn record_success(&mut self, domain: &str, selector: &str) {
        if let Some(site_context) = self.context_memory.site_contexts.get_mut(domain) {
            let success_rate = site_context.successful_selectors.entry(selector.to_string()).or_insert(0.0);
    ...
    pub async fn record_failure(&mut self, domain: &str, selector: &str) {
        if let Some(site_context) = self.context_memory.site_contexts.get_mut(domain) {
            let failure_count = site_context.failed_selectors.entry(selector.to_string()).or_insert(0);
            *failure_count += 1;
