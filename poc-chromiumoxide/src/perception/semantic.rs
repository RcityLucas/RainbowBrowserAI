...
impl SemanticAnalyzer {
    ...
    fn determine_page_intent(&self, html_content: &str) -> PageIntent {
        ...
        if content_lower.matches("<input").count() > 3 || content_lower.contains("submit") {
    ...
    pub async fn find_related_elements(
        &self,
        _target_selector: &str,
        _html_content: &str,
    ) -> Result<Vec<ElementRelationship>> {
    ...
    pub fn analyze_element_semantics(&self, element_text: &str, _context: &str) -> EntityType {
        ...
        } else if text_lower.contains("price")
