...
impl IntelligentActionTool {
    ...
    async fn execute_intelligent_action(
        &self,
        input: &IntelligentActionInput,
    ) -> Result<IntelligentActionOutput> {
        let action_id = Uuid::new_v4().to_string();
        let start_time = Instant::now();
        let mut logs = Vec::new();

        debug!(
            "Executing intelligent action: {} with target: {:?}",
            input.action_type, input.target
        );
        logs.push(format!(
            "Starting action: {} with target: {:?}",
            input.action_type, input.target
        ));
        ...
        for attempt in 0..input.retry_count {
            ...
            match self.try_execute_action(input, &mut logs).await {
                Ok(result) => {
                    info!(
                        "Action {} completed successfully on attempt {}",
                        action_id, attempts
                    );
                    logs.push(format!(
                        "Action completed successfully on attempt {}",
    ...
    async fn execute_double_click_action(
        &self,
        input: &IntelligentActionInput,
        logs: &mut Vec<String>,
    ) -> Result<ActionExecutionResult> {
        let selector = self.extract_selector(&input.target)?;
        logs.push(format!(
            "Double-clicking element with selector: {}",
            selector
        ));
    ...
    async fn execute_right_click_action(
        &self,
        input: &IntelligentActionInput,
        logs: &mut Vec<String>,
    ) -> Result<ActionExecutionResult> {
        let selector = self.extract_selector(&input.target)?;
        logs.push(format!(
            "Right-clicking element with selector: {}",
            selector
        ));
    ...
    async fn execute_type_action(
        ...
        let text = input
            .text
            .as_ref()
            .ok_or_else(|| anyhow::anyhow!("Text required for type action"))?;
    ...
    async fn execute_navigate_action(
        &self,
        input: &IntelligentActionInput,
        logs: &mut Vec<String>,
    ) -> Result<ActionExecutionResult> {
    ...
    async fn get_element_info(
        ...
        Ok(ElementInfoOutput {
            ...
            bounding_box: result.get("bounding_box").map(|bb| BoundingBoxOutput {
...
impl Tool for IntelligentActionTool {
    ...
    async fn validate_input(&self, input: &Self::Input) -> Result<()> {
        ...
        if input.action_type.to_lowercase() == "navigate"
            && input.url.is_none()
            && input.text.is_none()
        {
