...
use super::{LLMResponse, LLMError};
...
impl TaskPlan {
        ...
        if steps.is_empty() {
            ...
            steps.push(TaskStep {
                ...
                parameters: {
                    ...
                    params.insert("instruction".to_string(), serde_json::Value::String(content.to_string()));
                    params
                },
    ...
    fn parse_step_from_line(line: &str, step_num: usize) -> Result<Option<TaskStep>, LLMError> {
        ...
        let (step_type, action, description) = if line_lower.contains("navigate") || line_lower.contains("go to") || line_lower.contains("visit") {
            (TaskStepType::Navigate, "navigate", line)
        } else if line_lower.contains("click") {
            (TaskStepType::Click, "click", line)
        } else if line_lower.contains("type") || line_lower.contains("enter") || line_lower.contains("input") {
        ...
        } else if line_lower.contains("extract") || line_lower.contains("get") || line_lower.contains("find") {
        ...
        let step = TaskStep {
            ...
            parameters: {
                let mut params = HashMap::new();
                params.insert("instruction".to_string(), serde_json::Value::String(line.to_string()));
                params
            },
    ...
    pub fn validate(&self) -> Result<(), LLMError> {
        if self.steps.is_empty() {
            return Err(LLMError::InvalidResponse("Task plan has no steps".to_string()));
        }
...
impl TaskPlanExecutor {
    ...
    pub async fn execute_plan(&mut self, plan: &TaskPlan) -> Result<TaskExecutionSummary, LLMError> {
        ...
        for step_id in execution_order {
            if let Some(step) = plan.steps.iter().find(|s| s.id == step_id) {
                ...
                self.step_results.insert(step.id.clone(), step_result.clone());
        ...
        info!("Task plan {} completed: {}/{} steps successful", 
            plan.id, successful_steps, plan.steps.len());
    ...
    fn calculate_execution_order(&self, steps: &[TaskStep]) -> Result<Vec<String>, LLMError> {
        ...
        for step in steps {
            if !visited.contains(&step.id) {
                self.topological_sort(&step.id, steps, &mut visited, &mut temp_visited, &mut order)?;
            }
        }
    ...
    fn topological_sort(
        ...
        if temp_visited.contains(step_id) {
            return Err(LLMError::InvalidResponse("Circular dependency detected".to_string()));
        }
    ...
    async fn execute_navigate_step(&mut self, step: &TaskStep) -> Result<serde_json::Value, LLMError> {
        // Implementation for navigation
        info!("Executing navigate step: {}", step.description);
        Ok(serde_json::Value::String("Navigation completed".to_string()))
    }
    ...
    async fn execute_extract_step(&mut self, step: &TaskStep) -> Result<serde_json::Value, LLMError> {
        // Implementation for data extraction
        info!("Executing extract step: {}", step.description);
        Ok(serde_json::Value::String("Extraction completed".to_string()))
    }
    ...
    async fn execute_wait_step(&mut self, step: &TaskStep) -> Result<serde_json::Value, LLMError> {
        ...
        let wait_time = step.parameters.get("duration")
            .and_then(|v| v.as_u64())
            .unwrap_or(1000);
    ...
    async fn execute_scroll_step(&mut self, step: &TaskStep) -> Result<serde_json::Value, LLMError> {
    ...
    async fn execute_screenshot_step(&mut self, step: &TaskStep) -> Result<serde_json::Value, LLMError> {
        // Implementation for screenshots
        info!("Executing screenshot step: {}", step.description);
        Ok(serde_json::Value::String("Screenshot completed".to_string()))
    }
    
    async fn execute_validate_step(&mut self, step: &TaskStep) -> Result<serde_json::Value, LLMError> {
        // Implementation for validation
        info!("Executing validate step: {}", step.description);
        Ok(serde_json::Value::String("Validation completed".to_string()))
    }
    
    async fn execute_custom_step(&mut self, step: &TaskStep, custom_type: &str) -> Result<serde_json::Value, LLMError> {
        // Implementation for custom steps
        info!("Executing custom step ({}): {}", custom_type, step.description);
        Ok(serde_json::Value::String(format!("Custom step {} completed", custom_type)))
    }
