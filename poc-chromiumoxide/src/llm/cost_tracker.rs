...
impl CostTracker {
    ...
    pub fn track_usage(&mut self, response: &LLMResponse) {
        ...
        *self
        ...
        info!(
            "Tracked usage: {} tokens, ${:.4} cost for model {}",
    ...
    pub fn calculate_cost(&self, model: &str, prompt_tokens: u32, completion_tokens: u32) -> f64 {
        if let Some(pricing) = self.pricing.get(model) {
            ...
            let completion_cost =
                (completion_tokens as f64 / 1000.0) * pricing.completion_token_cost;
        ...
        for threshold in &self.limits.alert_thresholds {
            if let Some(daily_limit) = self.limits.daily_limit_usd {
                ...
                if usage_percent >= *threshold {
                    warn!(
                        "Daily cost alert: ${:.2} ({:.1}% of ${:.2} limit)",
                        daily_cost,
                        usage_percent * 100.0,
                        daily_limit
                    );
            ...
            if let Some(monthly_limit) = self.limits.monthly_limit_usd {
                ...
                if usage_percent >= *threshold {
                    warn!(
                        "Monthly cost alert: ${:.2} ({:.1}% of ${:.2} limit)",
                        monthly_cost,
                        usage_percent * 100.0,
                        monthly_limit
                    );
    ...
    pub fn export_csv(&self) -> String {
        let mut csv = String::from(
            "timestamp,model,prompt_tokens,completion_tokens,total_tokens,estimated_cost\n",
        );
