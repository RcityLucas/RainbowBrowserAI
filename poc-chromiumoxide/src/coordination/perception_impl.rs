...
use super::{
    ...
    session::{
        ActionAnalysis, ExecutionResult, IntelligentActionRequest, PageAnalysis, SessionContext,
        VerificationResult,
    },
...
pub struct RealCoordinatedPerceptionEngine {
    ...
    _context: Arc<RwLock<PerceptionContext>>,
...
impl RealCoordinatedPerceptionEngine {
    pub async fn new(
        ...
        let perception_engine =
            Arc::new(RwLock::new(PerceptionEngine::new(browser.clone()).await?));
    ...
    pub async fn analyze_for_action(
        ...
        let matching_elements: Vec<_> = elements
            ...
            .filter(|e| {
                e.text
                    .to_lowercase()
                    .contains(&action.target.to_lowercase())
                    || e.selector
                        .to_lowercase()
                        .contains(&action.target.to_lowercase())
            })
        ...
        Ok(ActionAnalysis {
            elements_found: matching_elements
                .iter()
                .map(|e| {
                    json!({
                ...
                .collect(),
            confidence,
            target_selector: matching_elements.first().map(|e| e.selector.clone()),
            alternative_selectors: matching_elements
                .iter()
                .skip(1)
                .take(2)
    ...
    pub async fn analyze_current_page(&self) -> Result<PageAnalysis> {
        ...
        let analysis = PageAnalysis {
            ...
            has_navigation: elements
                .iter()
                .any(|e| matches!(e.element_type, ElementType::NavigationLink)),
            page_type: perception
                .classify_page_type(&html)
                .map(|t| format!("{:?}", t))
    ...
    pub async fn cleanup(&self) -> Result<()> {
        info!(
            "Cleaning up perception engine for session: {}",
            self.session_id
        );
...
impl CoordinatedModule for RealCoordinatedPerceptionEngine {
    async fn initialize(&mut self, _context: &SessionContext) -> Result<()> {
        debug!(
            "Initializing perception module for session: {}",
            self.session_id
        );
        Ok(())
    }
    ...
    fn health_check(&self) -> ModuleHealth {
        let _ops_count = self.operations_count.blocking_read();
        let error_count = self.error_count.blocking_read();
        let _last_op = self.last_operation.blocking_read();
