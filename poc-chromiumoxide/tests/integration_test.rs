...
async fn test_tools_basic_functionality() -> Result<()> {
    ...
    let browser = std::sync::Arc::new(
        poc_chromiumoxide::browser::Browser::new(config).await?
    );
    ...
    let navigate_tool = std::sync::Arc::new(
        poc_chromiumoxide::tools::traits::DynamicToolWrapper::new(
    ...
    assert!(!tools.is_empty(), "Should have at least one registered tool");
