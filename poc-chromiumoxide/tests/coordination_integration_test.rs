...
use rainbow_poc_chromiumoxide::coordination::{
    ...
    EventBus, UnifiedCache, UnifiedStateManager,
};
use std::sync::Arc;
...
async fn test_perception_detects_tool_interface() -> Result<()> {
    ...
    for element in &elements {
        println!(
            "Element: {} - Type: {:?}",
            element.selector, element.element_type
        );
    ...
    assert!(
        tool_compatible > 0,
        "Should find at least one tool-compatible element"
    );

    // Now test coordinated action execution
    if let Some(first_element) = elements.first() {
        println!(
            "\nTesting coordinated action on: {}",
            first_element.selector
        );

        // Execute coordinated action (perception + tools)
        let result = context
            .execute_coordinated_action("click", &first_element.text)
            .await?;

        println!("Coordinated action result: {:?}", result);
        assert!(
            result["success"].as_bool().unwrap_or(false) || result["error"].is_string(),
...
async fn test_shared_instance_coordination() -> Result<()> {
    ...
    assert!(
        Arc::ptr_eq(&perception1, &perception2),
    ...
    assert_eq!(
        stats.cache_hits, 2,
        "Should have 2 cache hits for repeated access"
    );
...
async fn test_error_handling_with_fallback() -> Result<()> {
    ...
    let result: Result<String> =
        Err(anyhow::anyhow!("Primary failed")).or_fallback(|| Ok("Fallback succeeded".to_string()));
...
async fn test_module_coordinator() -> Result<()> {
    ...
    let context1 = coordinator
        .get_or_create_context("session1", browser1.clone())
        .await?;
    let context2 = coordinator
        .get_or_create_context("session2", browser2.clone())
        .await?;

    // Verify they are different contexts
    assert!(
        !Arc::ptr_eq(&context1, &context2),
        "Different sessions should have different contexts"
    );

    // Get the same context again
    let context1_again = coordinator
        .get_or_create_context("session1", browser1.clone())
        .await?;
    assert!(
        Arc::ptr_eq(&context1, &context1_again),
