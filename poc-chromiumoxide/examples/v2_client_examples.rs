...
impl V2ApiClient {
    ...
    async fn create_session(&mut self) -> Result<String> {
        ...
        Err(anyhow::anyhow!("Failed to create session: {:?}", api_response.error))
    }
        ...
        if api_response.success {
            if let Some(data) = api_response.data {
                ...
                if let Some(ref analysis) = data.analysis {
                    ...
                    println!("   Interactive: {} elements", analysis.interactive_elements.len());
                }
                return Ok(data);
            }
        }
        
        Err(anyhow::anyhow!("Navigation failed: {:?}", api_response.error))
    }
    
    // Execute an intelligent action
    async fn intelligent_action(&self, action_type: &str, target: &str) -> Result<IntelligentActionResult> {
        ...
        Err(anyhow::anyhow!("Intelligent action failed: {:?}", api_response.error))
    }
    ...
    async fn analyze_page(&self, analysis_type: Option<&str>) -> Result<PageAnalysis> {
        ...
        if api_response.success {
            if let Some(data) = api_response.data {
                ...
                println!("   Interactive elements: {}", data.interactive_elements.len());
                return Ok(data);
            }
        }
        
        Err(anyhow::anyhow!("Page analysis failed: {:?}", api_response.error))
    }
    ...
    async fn execute_tool(&self, tool_name: &str, parameters: serde_json::Value) -> Result<serde_json::Value> {
        ...
        Err(anyhow::anyhow!("Tool execution failed: {:?}", api_response.error))
    }
    ...
    async fn get_session_health(&self) -> Result<serde_json::Value> {
        ...
        Err(anyhow::anyhow!("Failed to get session health: {:?}", api_response.error))
    }
    ...
    async fn get_system_health(&self) -> Result<serde_json::Value> {
        ...
        Err(anyhow::anyhow!("Failed to get system health: {:?}", api_response.error))
    }
    ...
    async fn delete_session(&mut self) -> Result<()> {
        if let Some(session_id) = &self.session_id {
            ...
            return Err(anyhow::anyhow!("Failed to delete session: {:?}", api_response.error));
        }
...
async fn example_intelligent_actions() -> Result<()> {
    ...
    client.intelligent_action("type", "email input field").await?;
...
async fn example_tool_execution() -> Result<()> {
    ...
    client.execute_tool("take_screenshot", screenshot_params).await?;
...
async fn example_health_monitoring() -> Result<()> {
    ...
    // More operations
    client.intelligent_action("click", "navigation link").await?;
...
async fn example_error_handling() -> Result<()> {
    ...
    match client.intelligent_action("click", "non-existent-element").await {
        Ok(result) => {
            if !result.success {
