...
use poc_chromiumoxide::{
    ...
    tools::{
        ...
        // Interaction tools
        interaction::{ClickTool, FocusTool, HoverTool, SelectOptionTool, TypeTextTool},
        // Memory tools
        memory::{
            GetElementInfoTool, HistoryTrackerTool, PersistentCacheTool, ScreenshotTool,
            SessionMemoryTool,
        },
        // Navigation tools
        navigation::{GoBackTool, GoForwardTool, NavigateTool, RefreshTool, ScrollTool},
        registry::ToolRegistry,
        // Synchronization tools
        synchronization::{WaitForConditionTool, WaitForElementTool},
...
async fn register_all_tools(registry: &ToolRegistry, browser: Arc<Browser>) -> Result<()> {
    ...
    registry
        ...
        .await?;
    registry
        .register(Arc::new(DynamicToolWrapper::new(ScrollTool::new(
            browser.clone(),
        ))))
        .await?;
    registry
        .register(Arc::new(DynamicToolWrapper::new(RefreshTool::new(
            browser.clone(),
        ))))
        .await?;
    registry
        .register(Arc::new(DynamicToolWrapper::new(GoBackTool::new(
            browser.clone(),
        ))))
        .await?;
    registry
        .register(Arc::new(DynamicToolWrapper::new(GoForwardTool::new(
            browser.clone(),
        ))))
        .await?;
    ...
    // Interaction tools
    registry
        .register(Arc::new(DynamicToolWrapper::new(ClickTool::new(
            browser.clone(),
        ))))
        .await?;
    registry
        .register(Arc::new(DynamicToolWrapper::new(TypeTextTool::new(
            browser.clone(),
        ))))
        .await?;
    registry
        .register(Arc::new(DynamicToolWrapper::new(SelectOptionTool::new(
            browser.clone(),
        ))))
        .await?;
    registry
        .register(Arc::new(DynamicToolWrapper::new(HoverTool::new(
            browser.clone(),
        ))))
        .await?;
    registry
        .register(Arc::new(DynamicToolWrapper::new(FocusTool::new(
            browser.clone(),
        ))))
        .await?;
    ...
    registry
        ...
        .await?;
    registry
        .register(Arc::new(DynamicToolWrapper::new(ExtractLinksTool::new(
            browser.clone(),
        ))))
        .await?;
    registry
        .register(Arc::new(DynamicToolWrapper::new(ExtractDataTool::new(
            browser.clone(),
        ))))
        .await?;
    registry
        .register(Arc::new(DynamicToolWrapper::new(ExtractTableTool::new(
            browser.clone(),
        ))))
        .await?;
    registry
        .register(Arc::new(DynamicToolWrapper::new(ExtractFormTool::new(
            browser.clone(),
        ))))
        .await?;
    ...
    // Synchronization tools
    registry
        .register(Arc::new(DynamicToolWrapper::new(WaitForElementTool::new(
            browser.clone(),
        ))))
        .await?;
    registry
        .register(Arc::new(DynamicToolWrapper::new(
            WaitForConditionTool::new(browser.clone()),
        )))
        .await?;
    ...
    registry
        ...
        .await?;
    registry
        .register(Arc::new(DynamicToolWrapper::new(SessionMemoryTool::new(
            browser.clone(),
        ))))
        .await?;
    registry
        .register(Arc::new(DynamicToolWrapper::new(GetElementInfoTool::new(
            browser.clone(),
        ))))
        .await?;
    registry
        .register(Arc::new(DynamicToolWrapper::new(HistoryTrackerTool::new(
            browser.clone(),
        ))))
        .await?;
    registry
        .register(Arc::new(DynamicToolWrapper::new(PersistentCacheTool::new(
            browser.clone(),
        ))))
        .await?;
...
async fn demo_tool_chaining(registry: &ToolRegistry) -> Result<()> {
    ...
    let chain = vec![
        ...
        (
            "wait_for_element",
            json!({"selector": "body", "timeout_ms": 5000}),
        ),
